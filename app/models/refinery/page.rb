...
module Refinery
  class Page < Core::BaseModel
    ...
    attr_accessible :like_attributes, :members_only, :show_in_footer, :footer_order, :no_payment_required
    attr_accessible :legacy_id
    ...
    delegate :secret, to: :plan, prefix: true, allow_nil: true

    has_one :like, as: :likeable, :class_name => '::Refinery::Likes::Like'
    accepts_nested_attributes_for :like, :allow_destroy => true, :reject_if => proc{|a| a[:header].empty? }

    serialize :plan_ids, Array
    ...
    def plan_ids_attributes=(ids=[])
      self.plan_ids = ids.select do |k, v|
        v.to_s.include?("1")
      end.map do |k, v|
        k.to_i
      end
    ...
    def allowed_user?(user=nil)
      !members_only? || (
        members_only? &&
        user.present? &&
        user.active? &&
        users_plan_allowed?(user)
      )
    end

    def users_plan_allowed?(user)
      plan_ids.blank? || (
        plan_ids.present? &&
        plan_ids.include?(user.plan_id)
      )
    end

    def plans
      @plans ||= Refinery::Memberships::Plan.where(id: plan_ids)
    end

    def available_plan_badge
      plans.map do |p|
        p.title
      end.uniq[0]
    end
    ...
    def set_open_graph_defaults
      self.open_graph_title = self.title
      self.open_graph_description = self.content_for(:body)
      self.open_graph_image = 'apple-touch-icon-144x144.png'
    end
